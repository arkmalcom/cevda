...
const characteristics: Characteristic[] = [
    { icon: <p className="text-3xl">🍼</p>, text: 'Guardería fines de semana', content: guarderiaContent },
    { icon: <p className="text-3xl">⏰</p>, text: 'Abiertos de 6:00 AM a 6:00 PM', content: hoursContent },
    { icon: <p className="text-3xl">🏐</p>, text: 'Campamento de verano y de Navidad', content: campContent },
    { icon: <p className="text-3xl">📚</p>, text: 'Sala de tareas', content: salaTareasContent },
    { icon: <p className="text-3xl">👶</p>, text: 'Estimulación temprana desde los 45 días de edad', content: estimTempranaContent },
    { icon: <p className="text-3xl">🎭</p>, text: 'Clases extracurriculares', content: extraClassContent },
    { icon: <p className="text-3xl">🌳</p>, text: 'Espacios al aire libre', content: aireLibreContent },
    { icon: <p className="text-3xl">💡</p>, text: 'Aprendizaje dinámico', content: aprendizajeContent },
];

const Characteristics: React.FC = () => {
    return (
        <div className="flex flex-col space-y-3 bg-cover bg-center" style={{ backgroundImage: `url(${characteristicsBg})` }}>
            <h1 className="p-1 font-bold text-blue-800 text-center lg:text-xl font-welcome">Descubre los Servicios y Beneficios de Nuestro Centro Educativo</h1>
            <ul>
                {characteristics.map((item, index) => (
                    <li key={index} className="flex flex-col">
                        <span className="m-2 font-bold text-blue-800">{item.icon} {item.text}</span>
